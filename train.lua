...
--pwdistance = nn.PairwiseDistance(1)

--criterion
--local margin = 0.5

--criterion1=nn.HingeEmbeddingCriterion(margin)

ce1 = nn.CrossEntropyCriterion()
ce2 = nn.CrossEntropyCriterion()
criterion = nn.ParallelCriterion():add(ce1, 0.5):add(ce2, 0.5)

--criterion2 = nn.ParallelCriterion():add(ce1, 0.5):add(ce2, 0.5)

--criterion = nn.ParallelCriterion()
--criterion:add(criterion2, 0.25)
--criterion:add(criterion1, 0.75)
...
--encoder_dw_q:uniform(-0.08, 0.08)

encoder_w_q_2, encoder_dw_q_2=encoder_net_q_2:getParameters()
--encoder_w_q_2:uniform(-0.08, 0.08)
--encoder_dw_q_2:uniform(-0.08, 0.08)
...
---multimodal_w_2:uniform(-0.08, 0.08)
...
function JdJ(x)
	...
	--local yes_scores1 = scores1:sub(1, batch_size, 1, 1)/scores1:sub(1, batch_size, 1, 1):sum()
	--local yes_scores2 = scores2:sub(1, batch_size, 1, 1)/scores2:sub(1, batch_size, 1, 1):sum()
	 
	--distance = pwdistance:forward({yes_scores1, yes_scores2})

	--local labels1=torch.ones(scores1:size()[1])*-1	
	...
	-- pairwise distance backward using the HingeEmbedding criterion gradients grad[2]
	--local temp = pwdistance:backward({yes_scores1 ,yes_scores2}, grad[2])	
